...
export default function PlansTable({ data, isHome = false }: { data: PlansTableData, isHome?: boolean }) {
    ...
    return (
        <div className={isHome ? "py-16 sm:py-20" : "my-20"}>
            ...
            {data.title && (
                <div className="mb-10 text-left">
                    ...
                    >
                        <Award className="w-3.5 h-3.5 text-[var(--brand)] animate-pulse" />
                        Policymine Recommended Term Plans
                    </span>
                    <h2
                    ...
                    {data.introText && (
                        <p className="text-slate-500 mt-3 leading-relaxed max-w-3xl font-medium" style={{ fontSize: "var(--fs-body)", fontFamily: "var(--font-body)" }}>
                            {data.introText}
                        </p>
                    )}
                </div>
            ...
            >
                {(data.rows || []).map((row, i) => {
                    ...
                    return (
                        <motion.div
                            key={i}
                            className="group relative rounded-[32px] p-8 sm:p-10 border border-slate-100 bg-white shadow-[0_4px_25px_rgba(249,115,22,0.01)] overflow-hidden cursor-default"
                        ...
                        >
                                ...
                                <div className="absolute top-0 left-10 right-10 h-1 bg-gradient-to-r from-[var(--brand)] to-[var(--brand-200)] rounded-b-full opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                                ...
                                <div className="flex flex-col gap-8">
                                    ...
                                    <div className="grid lg:grid-cols-[1.5fr_1fr_1fr_1.1fr] gap-6 items-center">
                                        ...
                                        <div className="flex flex-col items-start lg:items-center text-left">
                                            ...
                                            <div className="inline-flex items-center gap-2 bg-[var(--brand-light)] border border-[var(--brand-100)] px-4 py-1.5 rounded-2xl shadow-sm">
                                                <ShieldCheck className="w-4.5 h-4.5 text-[var(--brand)] shrink-0" />
                                                <span className="text-sm sm:text-base font-extrabold text-[var(--brand-dark)] font-mono">{row.csr}</span>
                                            </div>
                                        </div>

                                        {/* Column 3: Policymine Rating */}
                                        <div className="flex flex-col items-start lg:items-center text-left">
                                            <p className="text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1.5" style={{ fontFamily: "var(--font-heading)" }}>
                                                Policymine Rating
                                            </p>
                                            {row.policymineRating ? (
                                                <StarRating rating={row.policymineRating} />
                                            ) : (
                                        ...
                                        <div className="text-left space-y-3.5">
                                            ...
                                            <div className="flex flex-wrap gap-2.5">
                                                {riderTags.map((tag, idx) => (
                                                    <motion.span
                                                        key={idx}
                                                        className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-2xl text-xs font-bold bg-slate-50 text-slate-600 border border-slate-100 group-hover:border-[var(--brand-100)] group-hover:bg-[var(--brand-light)] group-hover:text-slate-800 transition-all duration-300"
