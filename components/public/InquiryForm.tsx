                    ...
                    return (
                        <button
                            ...
                            style={{
                                ...
                                color: isDisabled ? "#CBD5E1" : value === item ? "#FFFFFF" : "var(--text-secondary)",
                                cursor: isDisabled ? "not-allowed" : "pointer"
                            }}
                        >
                            <span>{fmt(item)}</span>
                            {isDisabled && <span className="text-[8px] font-bold text-rose-400 uppercase select-none">Frozen</span>}
                        </button>
                    )
                })}
            </div>
...
export default function InquiryForm({ defaultType, compact = false }: { defaultType?: "term" | "health"; compact?: boolean }) {
    ...
    const emailVerified = emailOtpPhase === "verified" && !emailChanged
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-7 text-left">
            ...
            <div className="flex items-center gap-2 p-3 rounded-2xl bg-slate-50 border border-slate-100">
                ...
                <div className={`flex items-center gap-1.5 flex-1 px-2 py-1 rounded-xl text-xs font-bold transition-all ${emailVerified ? "text-emerald-700 bg-emerald-50" : "text-slate-400"}`}>
                    <Mail className={`w-3.5 h-3.5 shrink-0 ${emailVerified ? "text-emerald-500" : "text-slate-300"}`} />
                    <span>Email {emailVerified ? "✓" : "pending"}</span>
                </div>
                <div className="w-px h-5 bg-slate-200" />
            ...
            <div className="space-y-4">
                ...
                <div>
                    ...
                    <div className="flex gap-2">
                        <div className="relative flex-1">
                            <Input
                                ...
                                {...register("email")}
                                className={cn(fieldCls, emailVerified && "pr-10", emailChanged && "border-amber-400 focus:border-amber-500")}
                            />
                            {emailVerified && (
                                <ShieldCheck className="absolute right-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-emerald-500" />
                            )}
                        </div>
                        {/* Show Send OTP when idle, OR Re-verify when email changed after verification */}
                        {(emailOtpPhase === "idle" || emailOtpPhase === "sending") && emailValid && (
                        ...
                        {emailChanged && emailValid && (
                            <button type="button" onClick={() => { setVerifiedEmail(""); setEmailOtpPhase("idle"); setEmailOtpCode(""); setEmailOtpError("") }}
                                className="shrink-0 px-4 h-[46px] rounded-2xl text-xs font-bold flex items-center gap-1.5 transition-all bg-amber-500 hover:bg-amber-600 text-white shadow-sm hover:-translate-y-0.5"
                            >
                                <AlertCircle className="w-3.5 h-3.5" />
                                Re-verify
                    ...
                    {/* Changed-email warning */}
                    {emailChanged && (
                        <div className="mt-2 flex items-center gap-2 px-3 py-2 rounded-xl bg-amber-50 border border-amber-200">
                            <AlertCircle className="w-3.5 h-3.5 text-amber-500 shrink-0" />
                            <p className="text-xs font-semibold text-amber-700">Email changed — please re-verify to continue.</p>
                        </div>
                    )}
                    ...
                    {emailOtpError && <FieldError msg={emailOtpError} />}
                    {emailVerified && (
                        <p className="text-xs text-emerald-600 flex items-center gap-1.5 font-bold mt-1.5">
            ...
            <div className="space-y-3 pt-1">
                ...
                <p className="text-center text-[11px] text-slate-400 font-medium">
                    🔒 No spam · IRDAI Certified Experts · 100% Free
                </p>
            </div>
